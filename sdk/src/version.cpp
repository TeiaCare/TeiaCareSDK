namespace tc::sdk::info
{
extern const char* const name = "TeiaCoreSDK";
extern const char* const version = "1.0.0";

extern const char* const project_description = "TeiaCoreSDK is a collection of reusable C++ components";
extern const char* const project_url = "https://dev.azure.com/teiacare/Ancelia/_git/TeiaCoreSDK";

extern const char* const build_type = "Debug";
extern const char* const compiler_name = "GNU";
extern const char* const compiler_version = "12.3.0";

extern const char* const cxx_flags = " -march=native";
extern const char* const cxx_flags_debug = "-g";
extern const char* const cxx_flags_release = "-O3 -DNDEBUG";
extern const char* const cxx_standard = "20";

extern const char* const os_name = "Linux";
extern const char* const os_version = "6.2.0-26-generic";
extern const char* const os_processor = "x86_64";

}
